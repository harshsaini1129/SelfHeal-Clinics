...
import { Stethoscope, Menu, X, CalendarDays, LogOut, User, ChevronDown, Shield } from 'lucide-react';
...
export default function Navbar({ currentTab, onNavigate }: NavbarProps) {
  ...
  const activeCount = appointments.filter(appt => appt.status === 'Confirmed').length;
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-12 flex items-center justify-between h-20">
        ...
        >
          ...
          <div className="leading-none">
            <span className="block font-serif text-xl italic text-brand-charcoal">SelfHeal</span>
            <span className="block text-[10px] font-mono text-brand-clay font-bold uppercase tracking-widest mt-1">Hospitals</span>
          </div>
        </button>
          ...
          {user ? (
            <div className="relative">
              ...
              >
                ...
                <ChevronDown className={`w-3.5 h-3.5 text-brand-clay transition-transform duration-200 ${dropdownOpen ? 'rotate-180' : ''}`} />
              </button>
              ...
              {dropdownOpen && (
                <div className="absolute right-0 mt-3.5 w-72 bg-white border border-brand-olivelight rounded-2xl shadow-xl py-3.5 z-50 animate-fade-in text-left">
                  <div className="px-4 pb-3 border-b border-brand-stone mb-2.5">
                    <p className="text-[10px] uppercase font-bold text-brand-clay tracking-widest font-mono">My Account Dashboard</p>
                    <p className="text-sm font-bold text-brand-charcoal mt-1.5 truncate">
                      {userProfile?.fullName || user.displayName || 'Authorized Patient'}
                    </p>
                  ...
                  <div className="px-2 space-y-1">
                    ...
                    >
                      ...
                      {activeCount > 0 ? (
                        ...
                        <span className="text-[9px] text-brand-clay font-semibold">0 active</span>
                      )}
                    </button>
                    ...
                    {(user?.email?.toLowerCase() === 'harshsaini1129@gmail.com' || user?.email?.toLowerCase() === 'harshsaini1129@gamil.com') && (
                      <button
                        onClick={() => {
                          onNavigate('admin');
                          setDropdownOpen(false);
              ...
              <div className="bg-brand-stone rounded-2xl p-4 flex items-center justify-between border border-brand-olivelight/40 mb-1">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <span className="block text-[10px] text-brand-clay font-medium">{user.email}</span>
                  </div>
                </div>
                <button
                  onClick={() => {
